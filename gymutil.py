...
class AxesGeometry(LineGeometry):
    def __init__(self, scale=1.0, pose=None):
        verts = np.empty((3, 2), ti_vec3.dtype)
        verts[0][0] = (0, 0, 0)
        verts[0][1] = (scale, 0, 0)
        verts[1][0] = (0, 0, 0)
        verts[1][1] = (0, scale, 0)
        ...
        colors = np.empty(3, ti_vec3.dtype)
...
class WireframeBoxGeometry(LineGeometry):
    def __init__(self, xdim=1, ydim=1, zdim=1, pose=None, color=None):
        ...
        verts = np.empty((num_lines, 2), ti_vec3.dtype)
        # top face
        verts[0][0] = (x, y, z)
        verts[0][1] = (x, y, -z)
        verts[1][0] = (-x, y, z)
        ...
        colors = np.empty(num_lines, ti_vec3.dtype)
        colors.fill(color)
        self._colors = colors
...
def draw_line(p1, p2, color, gym, viewer, env):
    verts = np.empty((1, 2), dtype=ti_vec3.dtype)
    verts[0][0] = (p1.x, p1.y, p1.z)
    verts[0][1] = (p2.x, p2.y, p2.z)
    colors = np.empty(1, dtype=ti_vec3.dtype)
